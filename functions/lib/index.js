...
exports.analyzeVerse = exports.runAnalyzeVerse = void 0;
...
const qwenApiKey = (0, params_1.defineString)("QWEN_API_KEY");
const runAnalyzeVerse = async (requestData, qwenApiKeySecret) => {
    ...
    if (!apiKey) {
        throw new https_1.HttpsError("internal", "QWEN_API_KEY is not set.");
    }
...
- Desarrolla las analogías modernas con profundidad y riqueza descriptiva.`;
    ...
    try {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                if (error.status === 429 || error.status === 503) {
                    ...
                    throw new https_1.HttpsError("internal", "Hubo un error interno procesando el texto con Qwen Inteligencia Artificial.");
                }
            }
        }
        if (!resultText) {
        ...
        if (error instanceof https_1.HttpsError || error.httpErrorCode)
            throw error;
...
exports.runAnalyzeVerse = runAnalyzeVerse;
exports.analyzeVerse = (0, https_1.onCall)({ cors: true, timeoutSeconds: 540, memory: "1GiB" }, async (request) => {
    return await (0, exports.runAnalyzeVerse)(request.data, qwenApiKey.value());
});
