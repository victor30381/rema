...
exports.analyzeVerse = (0, https_1.onCall)({ cors: true }, async (request) => {
...
- Si el usuario pide 5 versículos, repite este proceso completo 5 veces. Sé sumamente exhaustivo y recuerda tu directiva principal de longitud: tu respuesta JAMÁS debe tener menos de 1500 palabras.`;
            break;
        case "Raíces":
            agentInstruction = `## Rol y Perfil:
...
- Extensión Mínima Obligatoria (CRÍTICO): Tu respuesta debe ser inmensamente descriptiva, detallada y exhaustiva. Bajo ninguna circunstancia tu respuesta debe tener menos de 1500 palabras. Debes expandir tus explicaciones, dar contexto histórico profundo, ejemplos detallados y razonamientos completos en cada uno de los 4 pasos para superar holgadamente esta cuota de palabras.
...
- Extensión Mínima Obligatoria (CRÍTICO): Tu respuesta debe ser una inmersión total y exhaustiva en el mundo antiguo. Bajo ninguna circunstancia tu respuesta debe tener menos de 1500 palabras. Debes expandir tus explicaciones históricas, detallar al máximo los paralelos literarios, contar anécdotas de la época y explicar a fondo la vida diaria en ese ecosistema cultural para superar holgadamente esta cuota de palabras. No asumas que ya sé las cosas; explícalo todo a profundidad.
...
- Extensión Mínima Obligatoria (CRÍTICO): Tu respuesta debe ser una reconstrucción inmensamente visual, detallada y minuciosa del entorno físico. Bajo ninguna circunstancia tu respuesta debe tener menos de 1500 palabras. Debes expandir tus descripciones del terreno, explicar a fondo cómo funcionaba cada artefacto o edificación, dar el contexto de los hallazgos epigráficos y pintar un cuadro hiperrealista de la vida cotidiana para superar holgadamente esta cuota de palabras.
...
- Extensión Mínima Obligatoria (CRÍTICO): Tu respuesta debe ser una inmersión monumental en la historia del pensamiento cristiano. Bajo ninguna circunstancia tu respuesta debe tener menos de 1500 palabras. Para lograr esto, debes expandir enormemente el contexto de cada debate, citar profusamente a los autores (explicando sus posturas con lujo de detalle), y narrar las tensiones teológicas como si fueran una novela fascinante. Tómate el tiempo de explicar conceptos como la "Cuadriga" o la "Wirkungsgeschichte" cada vez, para asegurarte de superar la cuota.`;
            break;
        case "El Mentor":
            agentInstruction = `## Rol y Perfil:
...
- Longitud Obligatoria: Tu respuesta DEBE tener un mínimo de 1500 palabras. Para lograr esto sin sonar repetitivo, debes profundizar en la etimología de las palabras originales (hebreo, arameo o griego), el contexto histórico y cultural del pasaje, y desglosar detalladamente por qué cada versión de la Biblia eligió traducir ciertas palabras de manera diferente.
...
- Desarrolla las analogías modernas con profundidad y riqueza descriptiva.`;
    ...
    try {
            ...
            try {
                ...
                if (error.status === 429 || error.status === 503) {
                    if (attempts >= maxAttempts) {
                        ...
                        throw new https_1.HttpsError("resource-exhausted", `Servidor saturado (${error.status}). Por favor, intenta de nuevo en unos minutos.`);
                    }
                    ...
                    throw new https_1.HttpsError("internal", "Hubo un error interno procesando el texto con Gemini IA.");
                }
            }
        }
        if (!result) {
            throw new https_1.HttpsError("internal", "No se pudo obtener respuesta de la IA.");
        }
        return {
        ...
        if (error.code && error.code.startsWith('functions/'))
            throw error;
        console.error("Unhadled Error:", error);
        throw new https_1.HttpsError("internal", "Hubo un error procesando el texto.");
    }
});
